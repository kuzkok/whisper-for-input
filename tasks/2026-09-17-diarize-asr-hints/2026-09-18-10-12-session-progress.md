# Сессия 2026-09-18 — реализация пунктов 1-5: initial_prompt/hotwords в /diarize
...
- `cli/diarize.sh` проверен против FastAPI-мок-сервера: с флагами мок получил оба поля (конечные переводы строк из файла срезаны), без флагов полей нет вовсе; несуществующий `--prompt-file` → внятная ошибка, exit 1.
...
- `cli/diarize.sh` — флаги `--prompt-file` / `--hotwords` готовы.
